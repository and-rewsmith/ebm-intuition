# main_multimodal_tail.py
import numpy as np
import matplotlib.pyplot as plt
import torch
import torch.nn as nn
import torch.optim as optim

# Fix random seeds for reproducibility
np.random.seed(0)
torch.manual_seed(0)

# --- Generate synthetic data WITH a MULTI-MODAL TAIL EVENT ---
n_samples_total = 1000
n_tail = 50  # Number of samples in the tail
n_main = n_samples_total - n_tail
n_per_main_cluster = n_main // 2

print(f"Generating data: {n_per_main_cluster} samples per main cluster, {n_tail} multi-modal tail samples.")

# Generate x values for each cluster
cluster1_x = np.random.normal(loc=-2.0, scale=0.2, size=(n_per_main_cluster, 1))
cluster2_x = np.random.normal(loc=2.0, scale=0.2, size=(n_per_main_cluster, 1))
cluster3_tail_x = np.random.normal(loc=6.0, scale=0.3, size=(n_tail, 1))  # Tail x values at x=6

# Generate corresponding y values - TAIL EVENT IS NOW MULTI-MODAL
y_cluster1 = cluster1_x.copy()  # y = x for cluster 1
y_cluster2 = cluster2_x.copy()  # y = x for cluster 2

# --- >>> Key Change: Create two modes for the tail y values <<< ---
n_tail_mode1 = n_tail // 2
n_tail_mode2 = n_tail - n_tail_mode1  # Handle odd n_tail
y_tail_mode1 = np.zeros((n_tail_mode1, 1))                # Mode 1: y = 0
y_tail_mode2 = np.full((n_tail_mode2, 1), 8.0)            # Mode 2: y = 8
y_cluster3_tail = np.vstack([y_tail_mode1, y_tail_mode2])
# --- <<< End Key Change >>> ---


# Stack x and y data separately
# Important: The y_cluster3_tail corresponds block-wise to cluster3_tail_x
# Shuffling later will mix them up properly pair-wise.
x_data = np.vstack([cluster1_x, cluster2_x, cluster3_tail_x])
y_data = np.vstack([y_cluster1, y_cluster2, y_cluster3_tail])  # y_data now has two modes in the tail

# Shuffle x and y together to maintain (x,y) pairs
indices = np.arange(x_data.shape[0])
np.random.shuffle(indices)
x_data = x_data[indices]
y_data = y_data[indices]

x_tensor = torch.tensor(x_data, dtype=torch.float32)
y_tensor = torch.tensor(y_data, dtype=torch.float32)  # This tensor now has y=0 AND y=8 for tail points

# --- Define MLP model (same architecture) ---


class MLP(nn.Module):
    def __init__(self):
        super(MLP, self).__init__()
        self.net = nn.Sequential(
            nn.Linear(1, 32),
            nn.ReLU(),
            nn.Linear(32, 1)
        )

    def forward(self, x):
        return self.net(x)

# --- Define simple EBM model (same architecture) ---


class EBM(nn.Module):
    def __init__(self):
        super(EBM, self).__init__()
        self.net = nn.Sequential(
            nn.Linear(1, 32),
            nn.ReLU(),
            nn.Linear(32, 1)
        )

    def forward(self, x):
        return self.net(x)


# --- Initialize models ---
mlp = MLP()
ebm = EBM()

# --- Optimizers ---
# Use MLP settings from user code, but potentially revert EBM LR if 1e-3 is unstable
mlp_opt = optim.Adam(mlp.parameters(), lr=1e-3)
ebm_opt = optim.Adam(ebm.parameters(), lr=1e-4)  # Reverted to 1e-4 as 1e-3 might be unstable for EBM

# --- Training loop for MLP ---
# MLP now sees the multi-modal tail data
print("Starting MLP training...")
mlp_epochs = 100000  # Keeping high epochs from user code
for epoch in range(mlp_epochs):
    mlp_opt.zero_grad()
    pred = mlp(x_tensor)
    loss = nn.MSELoss()(pred, y_tensor)
    loss.backward()
    mlp_opt.step()
    if (epoch + 1) % 10000 == 0:  # Adjusted print frequency
        print(f"MLP Epoch [{epoch+1}/{mlp_epochs}], Loss: {loss.item():.4f}")
print("MLP Training Finished.")

# --- Training loop for EBM ---
# EBM training still only depends on x_data distribution
ebm_epochs = 80000  # Keeping high epochs from user code
print(f"Starting EBM training for {ebm_epochs} epochs...")
for epoch in range(ebm_epochs):
    ebm_opt.zero_grad()
    real_energy = ebm(x_tensor)
    x_fake = torch.rand_like(x_tensor) * 16 - 8
    fake_energy = ebm(x_fake)
    # Keeping regularization for stability, could experiment removing it
    reg_loss = 0.1 * (real_energy**2 + fake_energy**2).mean()
    loss = (real_energy.mean() - fake_energy.mean()) + reg_loss
    loss.backward()
    ebm_opt.step()
    if (epoch + 1) % 10000 == 0:  # Adjusted print frequency
        print(f"EBM Epoch [{epoch+1}/{ebm_epochs}], Loss: {loss.item():.4f}")
print("EBM Training Finished.")


# --- Evaluate on a grid ---
x_grid = np.linspace(-5, 8, 700).reshape(-1, 1)
x_grid_tensor = torch.tensor(x_grid, dtype=torch.float32)

# MLP predictions
mlp_preds = mlp(x_grid_tensor).detach().numpy()

# EBM energies & probabilities (calculation unchanged)
ebm_energies = ebm(x_grid_tensor).detach().numpy()
max_neg_energy = np.max(-ebm_energies)
stable_exp_neg_energies = np.exp(-ebm_energies - max_neg_energy)
epsilon = 1e-9
log_partition_function = max_neg_energy + np.log(np.sum(stable_exp_neg_energies) + epsilon)
# Calculate final probabilities - REMOVED SCALING FACTOR
ebm_probs = np.exp(-ebm_energies - log_partition_function)

# --- Plot everything using subplots ---
fig, axs = plt.subplots(2, 1, figsize=(12, 10), sharex=True, gridspec_kw={
                        'height_ratios': [2, 2]})
fig.suptitle('Comparison: MLP vs EBM with Multi-Modal Tail Event (y=0 & y=8)', fontsize=16)  # Updated title

# --- Top Subplot (Densities and EBM) ---
# Should look similar to before - EBM models p(x)
ax = axs[0]
ax.hist(x_data, bins=70, density=True, alpha=0.5, label='True Data Distribution (Histogram)')
ax.plot(x_grid, ebm_probs, label='EBM Modeled p(x)', color='red')
ax.set_ylabel('Density')
# ax.set_ylim(0, 1) # Removed fixed ylim - let matplotlib decide
ax.grid(True)
ax.legend()
ax.set_title('EBM Density Estimation (Unaffected by y values)')

# --- Bottom Subplot (MLP Fit to Data) ---
# Scatter plot now shows TWO clusters of y values near x=6
ax = axs[1]
ax.scatter(x_data, y_data, s=10, alpha=0.3, color='blue', label='Raw Data Points (Tail y=0 & y=8)')
# MLP prediction line is expected to average the modes, predicting y~4 near x=6
ax.plot(x_grid, mlp_preds, label='MLP Predicted Mean E[y|x]', color='green', linewidth=2)
ax.set_xlabel('x')
ax.set_ylabel('y')
ax.grid(True)
ax.legend()
ax.set_title('MLP Regression Fit (Struggling with Multi-Modal Tail)')  # Updated title


# --- Final Adjustments and Save ---
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.savefig('mlp_ebm_comparison_multimodal_tail.png')  # New filename
print("Plot saved to mlp_ebm_comparison_multimodal_tail.png")
# plt.show()
